...
def build_chunk_filter(parsed: dict[str, Any]) -> models.Filter | None:
    metadata_filter = build_filter(parsed)
    if metadata_filter is None:
        return None

    parent_ids = retrieve_parent_ids(metadata_filter)
    if not parent_ids:
        return None
...
def retrieve_chunks(parsed: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    chunk_filter = build_chunk_filter(parsed)
...
def generate_answer_stream(
    ...
    system_prompt = ("""
        ...
        Always include brief source references using url when possible.
        The url should be http://127.0.0.1:8000/property-single.html?id= 
        followed by the parent_id from the retrieval context. 
        Always returns the markdown links [some text](url). 
        Can return multiple urls if multiple contexts are relevant.
